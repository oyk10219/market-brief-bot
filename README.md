...
Windows 작업 스케줄러 또는 GitHub Actions 수동 실행으로 텔레그램에 주식/IT 뉴스와 관심 종목 공시 링크를 보내는 MVP입니다.
...
- `TELEGRAM_DETAIL_MODE=compact`이면 텔레그램에는 핵심 요약, 오늘 볼 테마, 관심 종목/공시, 주요 기사 링크만 보냅니다. 상세 전체 목록은 `output/briefing_YYYYMMDD.md`에 저장됩니다.
...
섹션별 전체 뉴스 목록은 텔레그램으로 모두 보내지 않고 `output/briefing_YYYYMMDD.md`에 저장합니다.
